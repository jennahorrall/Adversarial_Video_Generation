...
def get_full_clips(data_dir, num_clips, num_rec_out=1):
    ...
    for clip_num, ep_dir in enumerate(dirs):
        ...
        for frame_num, frame_path in enumerate(clip_skipped_paths):   
            ...
            # check to make sure that log file is complete - some files stopped in the middle of run
            if frame.shape != (c.FULL_IMAGE_HEIGHT, c.FULL_IMAGE_WIDTH):
                get_full_clips(data_dir, num_clips)
            ...
            norm_frame = normalize_frames(frame_3, 100)
